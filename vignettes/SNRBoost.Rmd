...
col_seq <- RColorBrewer::brewer.pal(n=3, name="Set2")
...
fig <- ggplot(df_raw) + 
  ...
  scale_color_manual(values=c("black", col_seq)) + 
  labs(x=NULL, y=NULL) + 
...
fig1 <- ggplot(df_spec) +
  ...
  scale_color_manual(values=c("black",col_seq), labels=expression(y[Truth],x[1],x[2],x[3])) + 
...
## save plot 
ggsave(paste0("Figure1.jpg"), plt1, width = 9, height = 7,  dpi=500)
```
...
# Correlation
R_wa <- sapply(1:n_ensemble, function(i) cor(y_wa_ls[,i],df_mat[[i]]$y))
R_snr <- sapply(1:n_ensemble, function(i) cor(y_snr_ls[,i],df_mat[[i]]$y))
R_snr_freq <- sapply(1:n_ensemble, function(i) cor(y_snr_freq_ls[,i],df_mat[[i]]$y))

df_metric <- cbind(Group="MSE", data.frame(X1=MSE_wa, X2=MSE_snr, X3=MSE_snr_freq)) %>% 
  rbind(cbind(Group="Correlation",data.frame(X1=R_wa, X2=R_snr, X3=R_snr_freq))) %>% 
  gather(Model, value, 2:4)

summary(df_metric)

p2 <- ggplot(df_metric, aes(x=Model, y=value, fill=Model)) + 
    geom_boxplot() +
...
plt2 <- cowplot::plot_grid(fig2,fig3, ncol=2, labels=LETTERS, 
						               label_size=12,
                           label_fontfamily = "serif",
                           label_fontface = "bold",
                           label_colour = "black")
...
## save plot 
ggsave(paste0("Figure2.jpg"), plt2, width = 9, height = 5,  dpi=500)
```
